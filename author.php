<?php
  ...
  $query = mysql_query("SELECT *, name_" . SelectedLanguage . " AS name, biography_" . SelectedLanguage . " as biography FROM authors WHERE id = $id")
	or die("Error: SELECT");
  if (!mysql_num_rows($query)) error(); // нет автора с данным id
	
  $author = mysql_fetch_assoc($query);
  $sql = "SELECT books.*, books.name_" . SelectedLanguage . " as name, genre.name_" . SelectedLanguage . " AS genre_name 
