...
class phpthumb {
	...
	var $phpthumb_version = '1.7.2-200605312244';
...
//		}
	...
	function RenderOutput() {
		if (!$this->useRawIMoutput && !is_resource($this->gdimg_output)) {
			$this->DebugMessage('RenderOutput('.$filename.') failed because !is_resource($this->gdimg_output)', __FILE__, __LINE__);
			return false;
		}
	...
	function ImageMagickVersion() {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				if (eregi('^Version: (.*) (http|file)\:', $versionstring, $matches)) {
