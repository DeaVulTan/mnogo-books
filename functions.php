...
function CurrentURL() {
    ...
    $currentURL = $currentURL[0]."/index.php";
...
function alphabet() {
	...
	$sql = "SELECT UPPER(LEFT(name_" . SelectedLanguage . ",1)) as name FROM authors GROUP BY LEFT(name,1)";
...
echo $head;
  ...
  <tr>
	...
	<td width="100%" height="15">
	  <a href='about.php' class="designTBXMMenuText">{$lang['about']['header']}</a>
	</td>
  </tr>
EOD;
...
function get_genre_by_id($id) {
  $query = mysql_query("SELECT name_" . SelectedLanguage . " AS name FROM genre WHERE id=$id");
