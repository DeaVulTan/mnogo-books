...
<html>
	...
	РУС: Название книги (без кавычек):&nbsp;
	<input type="text" size='45' name="name_ru" value='<?= isset($book)?htmlspecialchars($book['name_ru']):"" ?>' ><br>
	<br>
	РУС: Аннотация к книге:<br>
	
	<textarea name='annotation_ru' cols=55 rows=10><?= isset($book)?htmlspecialchars($book['annotation_ru']):"" ?></textarea><br>
	<br>
        
        УЗБ: Название книги (без кавычек):&nbsp;
	<input type="text" size='45' name="name_uz" value='<?= isset($book)?htmlspecialchars($book['name_uz']):"" ?>' ><br>
	<br>
	УЗБ: Аннотация к книге:<br>
	
	<textarea name='annotation_uz' cols=55 rows=10><?= isset($book)?htmlspecialchars($book['annotation_uz']):"" ?></textarea><br>
	<br>
		
	Ссылка на книгу:&nbsp;<br>
	<input type="text" name="uri" size='60' value='<?= isset($book)?$book['uri']:"http://content3.mystream.uz/Books/" ?>' size=55><br>
	<br>
	...
	<select name="type">
        ...
        <option value="fb2" <?=$book["type"]=="fb2"?"selected":""; ?>>fb2</option>
	</select>
	<br><br>
	Выберите жанр:
	<select name="genre">
	<?php
		if (! ($result = mysql_query("SELECT *, name_ru as name FROM genre ORDER BY name")) )
			die("Cannot execute a query: " . mysql_error());
	...
	<td valign="top">
	  <form id='form_del_book' name='del_book' action='book.php' method="POST">
		...
		<?php
		  $query = mysql_query(
			"SELECT books.id AS b_id, books.name_ru AS b_name,books.date as b_date, authors.name_ru AS a_name FROM books, authors WHERE books.auth_id=authors.id ORDER BY books.name_ru");
