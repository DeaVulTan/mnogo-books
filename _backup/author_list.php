	...
	if (isset($_GET['bukva'])) {
	  ...
	  switch ($_GET['switch']) {
		case 0: { // авторы на данную букву
			...
			$sql = "SELECT authors.id,authors.name,count(books.id) AS add_info FROM authors,books 
			  WHERE authors.name REGEXP('^" . $bukva . "') AND books.auth_id=authors.id 
			  GROUP BY authors.id ORDER BY authors.name";
			break;
		}				
		case 1: { // книги на данную букву
			...
			$sql = "SELECT books.id,books.name,books.auth_id,authors.name AS add_info FROM authors,books 
			  WHERE books.name REGEXP('^" . $bukva . "') AND books.auth_id=authors.id 
			  GROUP BY books.id ORDER BY books.name";
			break;
		}	  
	  }		// end switch
	...
	elseif (isset($_GET['genre']) && is_numeric($_GET['genre'])) {
	  ...
	  switch ($_GET['switch']) {
		case 0: { // авторы данного жанра
			...
			$sql = "SELECT authors.id,authors.name,count(books.id) AS add_info FROM books,authors 
			  WHERE books.genre={$genre} AND authors.id=books.auth_id GROUP BY authors.id ORDER BY authors.name";
			break;
		}				
		case 1: { // книги данного жанра
			$discl_cent = $lang['discl_genre_books'];
			$in_title = $lang['genre_books'];
			$sql = "SELECT books.id,books.name,books.auth_id,authors.name AS add_info FROM books,authors 
			  WHERE books.genre={$genre} AND authors.id=books.auth_id GROUP BY books.id ORDER BY books.name";
			break;
		}
	  } 	// end switch
